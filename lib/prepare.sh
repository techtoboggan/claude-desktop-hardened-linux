...
_capp.on("browser-window-created",(e,w)=>{
  ...
  const _jsTemplate=[
    "(function(){",
      ...
      // Tooltips — make the scope of the toggle explicit. The chip's
      // left-click only affects Code mode and only future sessions;
      // conversation mode and already-running sessions are not touched.
      "if(_state.mode==='anthropic'){",
        "segA.title='Active: Anthropic (default)\\n' + ",
          "'New Code sessions and chat both use Anthropic';",
        "if(hasConfigured){",
          "const cm=_state.codeMode;",
          "segL.title='Click to enable 3rd party for Code mode\\n\\n' + ",
            "'Affects: NEW Code sessions you start after this click\\n' + ",
            "'Does NOT affect: this conversation, running Code sessions,\\n' + ",
            "'                 or any future chat messages\\n\\n' + ",
            "(cm?'Configured: '+(cm.model||'gateway')+' @ '+cm.baseUrl+'\\n\\n':'') + ",
            "'Right-click to route conversation mode through 3rd party too\\n(opens setup window; requires app restart)';",
        "}else{",
          "segL.title='No 3rd-party backend configured yet.\\n\\n' + ",
      ...
      "}else{",
        ...
        "let scopeNote,switchNote;",
        "if(_state.scope==='full-app'){",
          "scopeNote='Scope: FULL APP — both chat and Code mode\\n' + ",
            "       'route through your provider';",
          "switchNote='Click to disable and revert to Anthropic\\n(restarts the app)';",
        "}else if(_state.source==='env'){",
          "scopeNote='Scope: Code mode only (set via shell env var)\\n' + ",
            "       'Conversation/chat still uses Anthropic';",
          "switchNote='Cannot disable from the chip — unset\\n' + ",
            "          'ANTHROPIC_BASE_URL in your shell to revert';",
        "}else{",
          "scopeNote='Scope: NEW Code sessions only\\n' + ",
            "       'Conversation/chat still uses Anthropic\\n' + ",
            "       'Currently-running Code sessions keep their\\n' + ",
            "       'original backend until ended and restarted';",
          "switchNote='Click to disable for new Code sessions\\n' + ",
            "          '(running sessions are unaffected)';",
        "}",
        "segL.title='Active: '+providerName+(_state.detail?' @ '+_state.detail:'')+'\\n\\n' + ",
          "scopeNote+'\\n\\nRight-click to re-configure';",
        "segA.title=switchNote;",
      "}",
